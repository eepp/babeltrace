...
static GQuark q_statedump_soinfo;
static GQuark q_statedump_debug_link;
static GQuark q_statedump_build_id;
static GQuark q_statedump_start;
static GQuark q_dl_open;

struct proc_debug_info_sources {
	/* Hash table: base address to so info; owned by this */
	GHashTable *baddr_to_so_info;

	/* Hash table: IP to (struct debug_info_source *); owned by this */
		...
		uint64_t ip)
{
	...
	ret = so_info_lookup_source_location(so, ip, &src_loc, &found);
...
struct proc_debug_info_sources *proc_debug_info_sources_create(void)
{
	...
	proc_dbg_info_src->ip_to_debug_info_src = g_hash_table_new_full(
		NULL, NULL, NULL, (GDestroyNotify) debug_info_source_destroy);
		...
		struct proc_debug_info_sources *proc_dbg_info_src, uint64_t ip)
{
	...
	debug_info_src = g_hash_table_lookup(
		proc_dbg_info_src->ip_to_debug_info_src, (gpointer) key);
...
struct debug_info *debug_info_create(void)
{
	...
	debug_info->vpid_to_proc_dbg_info_src = g_hash_table_new_full(NULL,
		NULL, NULL, (GDestroyNotify) proc_debug_info_sources_destroy);
		...
		struct ctf_event_definition *event)
{
	...
	if (event_class->name == q_statedump_soinfo ||
			event_class->name == q_dl_open) {
		/* State dump/dlopen() */
		handle_statedump_soinfo_event(debug_info, event);
	} else if (event_class->name == q_statedump_start) {
		/* Start state dump */
		handle_statedump_start(debug_info, event);
	} else if (event_class->name == q_statedump_debug_link) {
		/* Debug link info */
		handle_statedump_debug_link_event(debug_info, event);
	} else if (event_class->name == q_statedump_build_id) {
		/* Build ID info */
...
{
	q_statedump_soinfo = g_quark_from_string("lttng_ust_statedump:soinfo");
	q_statedump_debug_link = g_quark_from_string("lttng_ust_statedump:debug_link)");
	q_statedump_build_id = g_quark_from_string("lttng_ust_statedump:build_id");
	q_statedump_start = g_quark_from_string("lttng_ust_statedump:start");
	q_dl_open = g_quark_from_string("lttng_ust_dl:dlopen");
