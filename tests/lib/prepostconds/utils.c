...
void run_in_comp_cls_init_defer(bt_self_component *self_comp,
		void *user_data)
{
	ppc_trigger_run_in_comp_cls_init_func user_func = user_data;

	user_func(self_comp);
}

static
void run_trigger(const struct ppc_trigger *trigger)
{
	switch (trigger->type) {
	case PPC_TRIGGER_FUNC_TYPE_BASIC:
		trigger->func.basic();
		break;
	case PPC_TRIGGER_FUNC_TYPE_RUN_IN_COMP_CLS_INIT:
...
static
void list_triggers(const struct ppc_trigger triggers[], size_t trigger_count)
{
	...
	for (i = 0; i < trigger_count; i++) {
		const struct ppc_trigger *trigger = &triggers[i];
		const char *type_str;

		/* Condition type */
		if (trigger->type == PPC_TRIGGER_TYPE_PRE) {
			type_str = "pre";
		} else if (trigger->type == PPC_TRIGGER_TYPE_POST) {
			type_str = "post";
		} else {
			abort();
		}
		...
		/* Name */
		escape_json_string(trigger->name, escaped_str);
		printf("\"name\":\"%s\",", escaped_str->str);

		/* Regex */
		escape_json_string(trigger->regex, escaped_str);
		printf("\"regex\":\"%s\"}", escaped_str->str);
...
void ppc_main(int argc, const char *argv[], const struct ppc_trigger triggers[],
